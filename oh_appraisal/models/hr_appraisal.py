...
class HrAppraisal(models.Model):
    ...
    employee_id = fields.Many2one(
        "hr.employee", string="Employee", help="Employee name"
    )
    ...
    final_interview = fields.Date(
        help="After sending survey link,you can" " schedule final interview date",
    )
    ...
    hr_manager = fields.Boolean(
        string="Manager",
        default=False,
        help="Whether the manager needs to " "attend survey",
    )
    ...
    hr_collaborator = fields.Boolean(
        ...
        default=False,
        help="To mention collaborators for" " the survey",
    )
    hr_colleague = fields.Boolean(
        string="Colleague", default=False, help="To mention colleagues for the survey"
    )
    ...
    collaborator_survey_id = fields.Many2one(
        ...
        help="Survey to send to the " "collaborator",
    )
    colleague_survey_id = fields.Many2one(
        "survey.survey",
        string="Select Colleague " "Opinion Form",
        help="Survey to send to the " "colleague",
    )
    ...
    final_evaluation = fields.Text(help="Final evaluation after the appraisal")
    ...
    tot_sent_survey = fields.Integer(
        string="Count Sent Questions", help="Number of Sent Questions"
    )
    ...
    check_draft = fields.Boolean(
        default=True,
        copy=False,
        help="Will be true when the appraisal in " "draft state",
    )
    check_cancel = fields.Boolean(
        copy=False,
        help="Will be true when the appraisal is " "canceled",
    )
    ...
    def _check_appraisal_deadline(self):
        ...
        if (
            self.appraisal_deadline <= fields.date.today()
            or self.appraisal_deadline == fields.date.today
        ):
            raise ValidationError(
                _("Appraisal deadline needs " "to be greater than today")
            )
    ...
    def fetch_appraisal_reviewer(self):
        ...
        if self.hr_manager and self.hr_manager_ids and self.manager_survey_id:
            appraisal_reviewers.append((self.hr_manager_ids, self.manager_survey_id))
        if self.hr_emp and self.emp_survey_id:
            appraisal_reviewers.append((self.employee_id, self.emp_survey_id))
        if (
            self.hr_collaborator
            and self.hr_collaborator_ids
            and self.collaborator_survey_id
        ):
        ...
        for appraisal_reviewers, survey_id in appraisal_reviewers_list:
            for reviewers in appraisal_reviewers:
                ...
                mail_content = (
                    ...
                    + reviewers.name
                    + ","
                    + "<br>Please fill out the following survey "
                    "related to "
                    + self.employee_id.name
                    + "<br>Click here to access the survey.<br>"
                    + baseurl
                    + str(url)
                    + "<br>Post your response for the appraisal till : "
